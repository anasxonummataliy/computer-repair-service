...
class AuthManager:
    ...
    async def require_auth(self, request: Request) -> Dict[str, Any]:
        ...
        if not user:
            raise HTTPException(
                status_code=401, detail="Avtorizatsiya talab qilinadi")
        return user
    ...
    async def require_role(self, request: Request, required_role: str) -> Dict[str, Any]:
        ...
        if user.get("role") != required_role:
            raise HTTPException(
                status_code=403, detail=f"Faqat {required_role} roli kerak")
        return user
    ...
    async def require_roles(self, request: Request, required_roles: list) -> Dict[str, Any]:
        ...
        if user.get("role") not in required_roles:
            raise HTTPException(
                status_code=403, detail=f"Quyidagi rollardan biri kerak: {', '.join(required_roles)}")
        return user
